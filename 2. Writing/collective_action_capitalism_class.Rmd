...
This article uses a quantitative methodology based on the [Social Survey Programme 2014 - Citizenship II (ISSP)](https://www.gesis.org/en/issp/modules/issp-modules-by-topic/citizenship/2014)[^5] (*N* = 49.087). This dataset applies a similar[^6] questionnaire in every country in which it is applied. The sampling procedure differs for each country: in some countries, partly simple random samples were applied, whereas in others, partly multistage stratified random samples. The data was collected in two ways. The first one was through interviews (face-to-face, Computer Assisted Personal interviews, or completed on the telephone) or Self-administered questionnaires (Computer Assisted self-interviews or Computer-Assisted web interviews). The analysis is mostly applied to individuals that are 18 years or more with some exceptions[^7]. After processing the dataset, the sample was reduced to 33.582 individuals nested in 34 countries.

[^5]: By the time this article has been written, another wave of the same thematic survey is under development and expected to fully realese in 2025.

[^6]: It is a similar questionary because the survey don't apply the exact same questions in every country. However, the questions are prepared to grasp and collect the same dimensions for every question applied.

[^7]: According to the ISSP, the exceptions were countries such as Finland, were individuals between 15 and 75 were surveyed, Japan, were 16 years old and older were surveyed, South Africa, were the respondents were 16 years or more, and Sweden, were the individuals were between 17 and 79 years old.
...
Prior to the statistical analysis, all the coding workflow was done using R and its many packages[^8].

[^8]: Most of the coding, wrangling, and analyses were used with tidyverse [@tidyverse], haven [@haven], sjPlot [@sjPlot], sjmisc [@sjmisc], sjlabelled [@sjlabelled], summarytools [@summarytools], car [@car], openxlsx [@openxlsx]. For presentation purposes stargazer [@stargazer], ggeffects [@ggeffects], and xtable [@xtable] were used.
...
\begin{table}[ht]
\centering
\caption{Dataframe descriptive statistics} 
\label{desc-table}
\begin{tabular}{rllrrrrl}
  \hline
 & var & label & n & NA.prc & mean & sd & range \\ 
  \hline
3 & acc2 & Dummy Collective Action & 33582 & 8.51 & 0.65 & 0.48 & 1 (0-1) \\ 
  7 & unionized & Participated in a Union & 36153 & 1.51 & 0.44 & 0.50 & 1 (0-1) \\ 
  5 & css & Social Class & 36706 & 0.00 & 6.67 & 2.45 & 8 (1-9) \\ 
  4 & cs\_sub & Subjective Social Class & 34224 & 6.76 & 0.96 & 0.74 & 2 (0-2) \\ 
  1 & AGE & Age & 36627 & 0.22 & 48.86 & 16.67 & 87 (15-102) \\ 
  2 & SEX & Sex & 36696 & 0.03 & 1.51 & 0.50 & 1 (1-2) \\ 
  6 & partner & Has a steady partner & 35142 & 4.26 & 0.69 & 0.46 & 1 (0-1) \\ 
   \hline
\end{tabular}
\end{table}
```
...
As many scholars have pointed out [@therbornClass21stCentury2012], the middle class has led many of the contemporary social movements. These movements, rather than being led by traditional organizations or parties, have been the result of the "unstable" place that such a class/group is in the class structure. Thus:
